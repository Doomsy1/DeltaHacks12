...
Tailscale allows you to keep all services private and access them securely over a VPN.

### Two Deployment Modes

**A) Simple Mode (Default):**
- Backend exposed publicly on port 8000
- Headless/Video services only accessible locally
- No Tailscale required

**B) Tailscale Mode:**
- All services kept private (only accessible via Tailscale)
- Access backend/headless/video over Tailscale network
- Optional: Use Tailscale Funnel for a single public endpoint later

### Setting Up Tailscale

1. **Generate a Tailscale Auth Key:**
   - Visit: https://login.tailscale.com/admin/settings/keys
   - Create an ephemeral or reusable key
...
3. **Enable Tailscale service in `docker-compose.prod.yml`:**
   - Uncomment the `tailscale` service block
...
6. **Check Tailscale admin console** - your node should appear

7. **Access services via Tailscale DNS:**
   - Backend: `http://vultr-node:8000`
   - Headless: `http://vultr-node:8001`
   - Video: `http://vultr-node:8002`

**IMPORTANT:** Never commit `TS_AUTHKEY` to git. It should only exist in your `.env` file.

For detailed instructions, see: `scripts/tailscale_setup.sh`
